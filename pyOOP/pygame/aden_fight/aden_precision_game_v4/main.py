...
class Level:
    ...
    def update(self, player):
        self.crumble.update()
        self.popup_spikes.update(player)
    ...
    def draw(self, camera):
        ...
        for popup in self.popup_spikes:
...
class Game:
    ...
    def music_for_level(self, index):
        ...
        if stage_number <= 20:
            return "red_shift.mp3"
        if stage_number == 21:
            return "city_of_the_wolf.mp3"
        if stage_number >= len(self.level_paths):
            return "final_boss_battle.mp3"
        if stage_number == 31:
            return "final_boss_battle.mp3"
        return "false_vacuum_decay.mp3"
    ...
    def respawn(self):
        ...
        self.player_shots.empty()
        # Keep the checkpoint label. Re-activate the closest matching checkpoint visually if possible.
        for cp in self.level.checkpoints:
            if abs(cp.rect.x - spawn_x) < 80 and abs(cp.rect.y - spawn_y) < 120:
                cp.activate()
    ...
    def handle_player_collisions(self):
        ...
        for popup in self.level.popup_spikes:
